...
export const getPostIdFromUrl = async (postUrl: string): Promise<string> => {
  // Clean the URL first - remove query parameters and fragments
  ...
  // Also handle URLs with additional path segments
  const postRegexWithPath = /^https:\/\/(?:www\.)?instagram\.com\/p\/([a-zA-Z0-9_-]+)\/.*/;
  const reelRegexWithPath = /^https:\/\/(?:www\.)?instagram\.com\/reels?\/([a-zA-Z0-9_-]+)\/.*/;

  console.log('Processing URL:', cleanUrl);

  if (shareRegex.test(cleanUrl)) {
    console.log('Detected Share URL');
    try {
      const reelId = await fetchReelIdFromShareURL(cleanUrl);
      return reelId;
    } catch (error) {
  ...
  let postMatch = cleanUrl.match(postRegex);
  if (postMatch?.[1]) {
    console.log('Matched Post ID:', postMatch[1]);
    return postMatch[1];
  }

  let reelMatch = cleanUrl.match(reelRegex);
  if (reelMatch?.[1]) {
    console.log('Matched Reel ID:', reelMatch[1]);
    return reelMatch[1];
  }

  // Try with additional path segments
  postMatch = cleanUrl.match(postRegexWithPath);
  if (postMatch?.[1]) {
    console.log('Matched Post ID with path:', postMatch[1]);
    return postMatch[1];
  }

  reelMatch = cleanUrl.match(reelRegexWithPath);
  if (reelMatch?.[1]) {
    console.log('Matched Reel ID with path:', reelMatch[1]);
    return reelMatch[1];
  }

  // Try a more flexible approach - extract any ID-like pattern
  const flexibleRegex = /instagram\.com\/(?:p|reel|reels)\/([a-zA-Z0-9_-]+)/;
  const flexibleMatch = cleanUrl.match(flexibleRegex);
  if (flexibleMatch?.[1]) {
    console.log('Matched flexible ID:', flexibleMatch[1]);
    return flexibleMatch[1];
  }

  console.error('No match found for URL:', cleanUrl);
  throw new Error('Unable to extract ID from URL: ' + cleanUrl);
};
...
export const isValidInstagramURL = (postUrl: string) => {
  ...
  const postRegex = /instagram\.com\/p\/([a-zA-Z0-9_-]+)/;
  const reelRegex = /instagram\.com\/reels?\/([a-zA-Z0-9_-]+)/;
  const shareRegex = /instagram\.com\/share\/([a-zA-Z0-9_-]+)/;
